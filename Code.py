...
connection = pymysql.connect(host='localhost',port=3307, user='root', password='',database="sra")
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'K' and ad_password == 'A':
                st.success("Welcome Allan")
                # Display Data
                ...
                fig = px.pie(df, names=labels, title='Predicted Field according to the Skills')
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, names=labels, title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                st.plotly_chart(fig)
